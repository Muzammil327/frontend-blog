...
export default function MenuLeft() {
  ...
  // filter by catgeory
  const catgeory = postMetadata.filter((item) => item.cat === "next auth");
  ...
  return (
    <>
      ...
      <div className="md:col-span-2 bg-gray-200 mt-20 md:block hidden pt-10 min-h-[800px]">

        {uniqueCategories.map((category, index) => (
          <div key={category}>
            <Link
              className=" text-black block py-3 px-4 hover:bg-gray-100"
              href={`/course/next-auth/${category
                .toLowerCase()
                .replace(/\s+/g, "-")}`}
            >
              {category}
